    ...
    def import_csv(self, request):
        if request.method == "POST":
            ...
            for x in csv_data:
                ...
                # ⬇️ prevent IndexError on empty/short lines
                if not x.strip() or len(fields) < 7:
                    continue
                ...
                try:
                 ...
                 # send email even if update_or_create raised earlier
                to_email = (fields[4] or "")
                if to_email:
                    subject = "Vos accès à la plateforme"
                    message = (
                        f"Bonjour {fields[2]} {fields[3]},\n\n"
                        f"Votre compte a été créé/mis à jour.\n"
                        f"Identifiant : {fields[1]}\n"
                        f"Mot de passe : {pass_word}\n\n"
                        f"Merci de vous connecter.\n"
                        f"Cordialement."
                    )
                    try:
                        send_mail(
